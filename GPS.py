  ...
  try:
    ...
    while True:
      ...
      if str(gpsd.fix.latitude) != 'nan' and str(gpsd.fix.latitude) != '0.0':
        GPIO.output(22,True)
        try:
          resp = requests.get(gps_url+'track_latitude={0:.6f}&track_longitude={1:.6f}&track_speed={2:.2f}&car_id={3}'.format(gpsd.fix.latitude,gpsd.fix.longitude,gpsd.fix.speed,id), timeout=1.001)
